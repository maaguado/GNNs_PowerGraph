...
class DConvFix(MessagePassing):
    def __init__(self, in_channels, out_channels, K, bias=True):
        super(DConvFix, self).__init__(aggr="add", flow="source_to_target")
        assert K > 0
    ...
    def forward(self, X: torch.FloatTensor, edge_index: torch.LongTensor, edge_weight: torch.FloatTensor) -> torch.FloatTensor:
        ...
        T = edge_weight.size(1)

        # Convertir edge_index y edge_weight en tensores 3D
        num_edges = edge_index.size(1)
        #print("edge_attr size", edge_weight.size())
        adj_mats = []
        for t in range(T):
            adj_mat = to_dense_adj(edge_index, edge_attr=edge_weight[:,t,:])
            adj_mats.append(adj_mat)

        #print("X size", X.size())
        # Concatenar y ajustar las dimensiones

        adj_mats = torch.cat(adj_mats, dim=0).view(T, X.size(0), X.size(0), -1)
        adj_mats = adj_mats.mean(dim=-1)


        deg_out = torch.matmul(adj_mats, torch.ones(size=(T, adj_mats.size(1), 1)).to(X.device)).squeeze(-1)
        deg_in = torch.matmul(torch.ones(size=(T, 1, adj_mats.size(1))).to(X.device), adj_mats).squeeze(1)

        deg_out_inv = torch.reciprocal(deg_out)
        deg_in_inv = torch.reciprocal(deg_in)

        row, col = edge_index.view(2, -1)
        norm_out = deg_out_inv[:, row].view(-1)
        norm_in = deg_in_inv[:, row].view(-1)

        reverse_edge_index = adj_mats.transpose(1, 2)
