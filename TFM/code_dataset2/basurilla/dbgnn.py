...
class DBGNN(torch.nn.Module):
    ...
    def __init__(
        self,
        target: int,
        num_features: list[int], #Ejemplo de num_features con dos  = [n_features_entrada, n_features_salida]
        hidden_dims: list[int], #Ejemplo de hidden_dims con dos  = [30, 30]
        p_dropout: float = 0.0
    ):
        super().__init__()
        ...
        # Linear layer
        self.lin = torch.nn.Linear(self.hidden_dims[-1], target)
